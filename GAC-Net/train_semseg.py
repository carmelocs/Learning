...
def parse_args():
    ...
    parser.add_argument('--batchSize',
                        type=int,
                        default=24,
                        help='input batch size [default: 24]')
    ...
    parser.add_argument(
        ...
        help='learning rate for training \
            [default: 0.001 for Adam, 0.01 for SGD]')
...
def main(args):
    ...
    for epoch in range(init_epoch, args.epoch):
        ...
        # if epoch % 10 == 0:
        #     train_metrics, train_hist_acc, cat_mean_iou = test_seg(
        #         model, dataloader, seg_label_to_cat)
        #     print('Epoch %d  %s loss: %f accuracy: %f  meanIOU: %f' %
        #           (epoch, blue('train'), history['loss'][-1],
        #            train_metrics['accuracy'], np.mean(cat_mean_iou)))
        #     logger.info('Epoch %d  %s loss: %f accuracy: %f  meanIOU: %f' %
        #                 (epoch, 'train', history['loss'][-1],
        #                  train_metrics['accuracy'], np.mean(cat_mean_iou)))

        test_metrics, test_hist_acc, cat_mean_iou = test_seg(
            model, testdataloader, seg_label_to_cat)
        ...
        if test_metrics['accuracy'] > best_acc:
            ...
            torch.save(
                model.state_dict(),
                '%s/GACNet_%.3d_%.4f.pth' % (checkpoints_dir, epoch, best_acc))
