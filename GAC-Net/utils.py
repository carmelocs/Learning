...
def save_checkpoint(epoch, train_accuracy, test_accuracy, model, optimizer,
                    path):
...
def compute_iou(pred, target, iou_tabel=None):
    ...
    for j in range(pred.size(0)):
        ...
        for cat in np.unique(batch_target):
            intersection = np.sum((batch_target == cat)
                                  & (batch_choice == cat))
            union = float(np.sum((batch_target == cat)
                                 | (batch_choice == cat)))
...
def test_seg(model, loader, catdict, num_classes=13):
    ...
    metrics = defaultdict(lambda: list())
    hist_acc = []
    for batch_id, (points, target) in tqdm(enumerate(loader),
                                           total=len(loader),
                                           smoothing=0.9):
